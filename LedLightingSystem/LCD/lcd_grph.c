...
/******************************************************************************
** Function name:		lcd_draw_widget
**
** Descriptions:		Draws a widget on the lcd display
**
** parameters:			widget
** Returned value:	None
** 
...
void lcd_draw_widget(Widget * widget){
	...
	lcd_color_t fontColour;
	
	if(widget->locked == TRUE){
		unpressedColour = SILVER;
		currentColour = SILVER;
		fontColour = DARK_GRAY;
	}
	else{
		unpressedColour = widget->unpressedColour;
		currentColour = widget->currentColour;
		fontColour = BLACK;
	}
	
	/* calculate center point for symbols */
	xCenter = widget->x1Pos - ((widget->x1Pos-widget->x2Pos)/2);
	yCenter = widget->y1Pos - ((widget->y1Pos-widget->y2Pos)/2);
	
	switch(widget->type){
		case POWER :
			lcd_fillRect(widget->x1Pos, widget->y1Pos, widget->x2Pos, widget->y2Pos, currentColour);
			lcd_fontColor(fontColour, currentColour);
			lcd_putString(widget->textX1Pos,widget->textY1Pos, (unsigned char*)widget->text);
			break;
		...
		case DIM :
			widget->state == UNPRESSED?
				(lcd_fillRect(widget->x1Pos, widget->y1Pos, widget->x2Pos, widget->y2Pos, unpressedColour)) :
				(lcd_fillRect(widget->x1Pos, widget->y1Pos, widget->x2Pos, widget->y2Pos, widget->pressedColour));
			lcd_minusSign(xCenter, yCenter, fontColour);
			break;
		...
		case BRIGHTEN :
			widget->state == UNPRESSED?
				(lcd_fillRect(widget->x1Pos, widget->y1Pos, widget->x2Pos, widget->y2Pos, unpressedColour)) :
				(lcd_fillRect(widget->x1Pos, widget->y1Pos, widget->x2Pos, widget->y2Pos, widget->pressedColour));
			lcd_plusSign(xCenter, yCenter, fontColour);
			break;
		...
		case CONFIG :
			widget->state == UNPRESSED?
				(lcd_fillRect(widget->x1Pos, widget->y1Pos, widget->x2Pos, widget->y2Pos, unpressedColour)) :
				(lcd_fillRect(widget->x1Pos, widget->y1Pos, widget->x2Pos, widget->y2Pos, widget->pressedColour));
			lcd_fontColor(fontColour, unpressedColour);
			lcd_putString(widget->textX1Pos,widget->textY1Pos, (unsigned char*)widget->text);
			break;
...
{
		unsigned int offset = 10; // standard offset for all plus signs
		lcd_line(x0,y0+offset,x0,y0-offset, colour);
		lcd_line(x0-offset,y0,x0+offset,y0, colour);
}
